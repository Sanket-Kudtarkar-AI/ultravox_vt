...
import {
    ...
    ChevronUp, PieChart, Activity, ArrowUpRight, Info, FileText,
...
const CampaignResults = ({
    ...
    // Effect to fetch analysis status for completed calls
    useEffect(() => {
        const completedContacts = contacts.filter(
            contact => contact.status === 'completed' && contact.call_uuid
        );

        if (completedContacts.length > 0) {
            // Get all call UUIDs from completed contacts
            const callUuids = completedContacts.map(contact => contact.call_uuid);

            // Use the batch API to get all statuses at once
            const fetchBatchStatus = async () => {
                try {
                    setCheckingAnalysis(true);

                    const response = await fetch(`${API_BASE_URL}/call_analysis_status_batch`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        },
                        body: JSON.stringify({call_uuids: callUuids})
                    });

                    if (!response.ok) {
                        throw new Error(`Error fetching batch analysis status: ${response.status}`);
                    }
                    ...
                    if (data.status === 'success') {
                        ...
                        Object.entries(data.results).forEach(([callUuid, statusData]) => {
                            newStatuses[callUuid] = {
                                callUuid,
                                vtCallId: statusData.ultravox_id,
                                hasAnalysis: statusData.is_complete,
                                details: {
                                    transcript: statusData.has_transcript,
                                    recording: statusData.has_recording,
                                    summary: statusData.has_summary
                                },
                        ...
                        // Update our state
                        setAnalysisStatus(newStatuses);

                        // Calculate and update progress
                        const completeCount = Object.values(newStatuses).filter(status => status.hasAnalysis).length;
                        const progress = Math.round((completeCount / completedContacts.length) * 100);
                        setAnalysisProgress(progress);
                    } else {
                        console.error('Error in batch API response:', data.message);
                        // Fall back to individual checks if batch fails
                        fetchAnalysisStatus(callUuids);
                    }
                } catch (error) {
                    console.error('Error fetching batch analysis status:', error);
                                ...
                                if (contactIndex !== -1) {
                                    ...
                                    try {
                                        ...
                                        if (updatedContacts[contactIndex].additional_data) {
                                            // Check if additional_data is already an object or a string
                                            additionalData = typeof updatedContacts[contactIndex].additional_data === 'object' ?
                                                updatedContacts[contactIndex].additional_data :
                                                JSON.parse(updatedContacts[contactIndex].additional_data);
                                        }
                                    } catch (e) {
    ...
    // Function to fetch analysis status for completed calls
    const fetchAnalysisStatus = async (callUuids) => {
        if (!callUuids || callUuids.length === 0) return;

        setCheckingAnalysis(true);

        try {
            // Batch the call UUIDs to avoid making too many API calls at once
            const batchSize = 10;
            const statuses = {...analysisStatus};

            for (let i = 0; i < callUuids.length; i += batchSize) {
                const batchUuids = callUuids.slice(i, i + batchSize);

                // Use Promise.all to fetch all statuses in parallel
                await Promise.all(batchUuids.map(async (callUuid) => {
                    try {
                        // Fetch the analysis status from our API endpoint
                        const response = await fetch(`${API_BASE_URL}/call_analysis_status/${callUuid}`);

                        if (response.ok) {
                            const data = await response.json();

                            if (data.status === 'success' && data.analysis_status) {
                                const analysisData = data.analysis_status;

                                // Determine if VT call ID is available and all components are ready
                                const hasAnalysis = analysisData.is_complete;
                                const vtCallId = analysisData.ultravox_id;
                                ...
                                statuses[callUuid] = {
                                    ...
                                    hasAnalysis,
                                    details: {
                                        transcript: analysisData.has_transcript,
                                        recording: analysisData.has_recording,
                                        summary: analysisData.has_summary
                                    },
                                    lastChecked: analysisData.last_checked
                                };
                            } else {
                                // If we don't have data, fall back to checking the mapping
                                await checkAnalysisMappingFallback(callUuid, statuses);
                            }
                        } else {
                            // If the endpoint fails, fall back to checking the mapping
                            await checkAnalysisMappingFallback(callUuid, statuses);
                        }
                    } catch (error) {
                        console.error(`Error fetching analysis status for call ${callUuid}:`, error);
                        // Fall back to checking the mapping if our API fails
                        await checkAnalysisMappingFallback(callUuid, statuses);
                    }
                }));
            }

            // Update our state with all the fetched statuses
            setAnalysisStatus(statuses);

            // Calculate and update analysis progress
            const completedCalls = contacts.filter(contact => contact.status === 'completed').length;
            if (completedCalls > 0) {
                const completedAnalyses = Object.values(statuses).filter(status => status && status.hasAnalysis).length;
                const progress = Math.round((completedAnalyses / completedCalls) * 100);
                setAnalysisProgress(progress);
            } else {
    ...
    // Fallback function to check analysis components directly
    const checkAnalysisMappingFallback = async (callUuid, statuses) => {
        try {
            // Get the VT call ID mapping
            const mappingResponse = await fetch(`${API_BASE_URL}/call_mapping/${callUuid}`);

            if (!mappingResponse.ok) {
                statuses[callUuid] = {callUuid, hasAnalysis: false, reason: 'mapping-error'};
                return;
            }
            ...
            if (mappingData.status !== 'success' || !mappingData.mapping.ultravox_call_id) {
                statuses[callUuid] = {callUuid, hasAnalysis: false, reason: 'no-mapping'};
                return;
            }
            ...
            console.error(`Error in analysis mapping fallback for call ${callUuid}:`, error);
            statuses[callUuid] = {
                callUuid,
                hasAnalysis: false,
    ...
    // Main function to check analysis availability
    const checkAnalysisAvailability = async () => {
        try {
            ...
            // Use the batch API
            await fetchAnalysisStatus(callUuids);

            // Schedule a retry if not all analyses are available yet
            if (completedContacts.length > 0 &&
                analysisProgress < 100 &&
                Object.values(analysisStatus).filter(s => s?.hasAnalysis).length < completedContacts.length) {
    ...
    // Add this utility function at the top of your component (before your useEffect hooks)
...
// Replace your fetchCampaignData function with this updated version
    const fetchCampaignData = async () => {
        if (!campaign || !campaign.campaign_id) {
            console.error("Campaign or campaign_id is undefined:", campaign);
        ...
        try {
            // Log the URLs we're trying to fetch for debugging
            const statsUrl = `${API_BASE_URL}/campaigns/${campaign.campaign_id}/stats`;
            const contactsUrl = `${API_BASE_URL}/campaigns/${campaign.campaign_id}/contacts`;
            ...
            // Use fetchWithRetry for contacts too
            const contactsResponse = await fetchWithRetry(contactsUrl, {
                headers: {'Content-Type': 'application/json'}
            });
            ...
            if (contactsData.status === 'success') {
                ...
                if (contactsWithCallUuids.length > 0) {
                    try {
                        ...
                        // Continue execution even if call details fail
                    }
                }
            } else {
            ...
            if (err.name === 'TypeError' && err.message === 'Failed to fetch') {
                setError('Network error: Cannot connect to the server. Please check your internet connection and server status.');
                ...
                <div className="flex items-center space-x-3">
                    <div className="flex items-center mr-3">
                        ...
                        <div className="relative inline-flex items-center cursor-pointer">
                            ...
                            />
                            <div
                                className="w-11 h-6 bg-dark-600 peer-focus:outline-none rounded-full peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary-600"></div>
                        </div>
            ...
            {!loading && campaignStats && (
                <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        {/* Key Metrics */}
                        <div
                            className="bg-dark-700/30 p-5 rounded-lg border border-dark-600 hover:border-primary-500/30 transition-colors shadow-md">
                            <h4 className="font-medium text-white mb-4 flex items-center">
                                <div
                                    className="w-8 h-8 rounded-full bg-blue-900/30 border border-blue-500/20 flex items-center justify-center mr-3">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-dark-800/50 p-4 rounded-lg">
                                    ...
                                    <div
                                        className="text-2xl font-bold text-white">{campaignStats.total_contacts || 0}</div>
                                </div>

                                <div className="bg-dark-800/50 p-4 rounded-lg">
                                    <div className="text-sm text-gray-400 mb-1">Completion Rate</div>
                                    <div
                                        className="text-2xl font-bold text-white">{campaignStats.completion_rate || 0}%
                                    </div>
                                </div>

                                <div className="bg-dark-800/50 p-4 rounded-lg">
                                    <div className="text-sm text-gray-400 mb-1">Success Rate</div>
                                    <div className="text-2xl font-bold text-white">{campaignStats.success_rate || 0}%
                                    </div>
                                </div>
                                ...
                                <div className="bg-dark-800/50 p-4 rounded-lg">
                                    ...
                                    <div
                                        className="text-2xl font-bold text-white">{campaignStats.average_call_duration || 0}s
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-4 mt-4">
                                <div className="bg-green-900/20 border border-green-900/30 p-3 rounded-lg">
                                    ...
                                    <div
                                        className="text-xl font-bold text-white">{campaignStats.completed_contacts || 0}</div>
                                </div>
                                ...
                                <div className="bg-yellow-900/20 border border-yellow-900/30 p-3 rounded-lg">
                                    ...
                                    <div
                                        className="text-xl font-bold text-white">{campaignStats.no_answer_contacts || 0}</div>
                                </div>

                                <div className="bg-red-900/20 border border-red-900/30 p-3 rounded-lg">
                                    <div className="text-xs text-red-400 mb-1">Failed</div>
                                    <div
                                        className="text-xl font-bold text-white">{campaignStats.failed_contacts || 0}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div
                            ...
                            <h4 className="font-medium text-white mb-4 flex items-center">
                                <div
                                    className="w-8 h-8 rounded-full bg-primary-900/30 border border-primary-500/20 flex items-center justify-center mr-3">
                            ...
                            <div className="mb-6">
                                <div className="flex justify-between text-sm mb-1">
                                    ...
                                    <span
                                        className="text-white">{campaignStats.progress || campaignStats.completion_rate || 0}%</span>
                                </div>
                                ...
                                <div className="w-full bg-dark-600 rounded-full h-4 mb-1">
                                    <div
                                        ...
                                        style={{width: `${campaignStats.progress || campaignStats.completion_rate || 0}%`}}
                                    ></div>
                                </div>
                            </div>
                            ...
                            {(campaign.status === 'completed' ||
                                campaignStats.completed_contacts > 0 ||
                                campaignStats.analysis_progress > 0 ||
                                analysisProgress > 0) && (
                                <div className="mb-6">
                                    <div className="flex justify-between text-sm mb-1">
                                        <span className="text-gray-400">Analysis Ready</span>
                                        <span
                                            className="text-white">{campaignStats.analysis_progress || analysisProgress || 0}%</span>
                                    </div>

                                    <div className="w-full bg-dark-600 rounded-full h-4 mb-1">
                                        <div
                                            className="bg-gradient-to-r from-accent-600 to-accent-400 h-4 rounded-full transition-all duration-500"
                                            style={{width: `${campaignStats.analysis_progress || analysisProgress || 0}%`}}
                                        ></div>
                                    </div>
                            ...
                            <div className="space-y-3">
                                ...
                                {campaignStats.schedule_date && (
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span
                                            className="text-white">{formatDateTime(campaignStats.schedule_date)}</span>
                                    </div>
                                )}
                                        ...
                                        <td className="px-4 py-4">
                                            ...
                                            {(() => {
                                                ...
                                                if (contact.additional_data) {
                                                    try {
                                                        // Check if additional_data is already an object or a string
                                                        const additionalData = typeof contact.additional_data === 'object' ?
                                            ...
                                            {(() => {
                                                ...
                                                if (contact.additional_data) {
                                                    try {
                                                        ...
                                                        } else if (additionalData.call_log_duration) {
                                                            return `${additionalData.call_log_duration}s`;
                                                        }
                                                    } catch (e) {
                                                    ...
                                                    {analysisStatus[contact.call_uuid] ? (
                                                        <div className="flex items-center space-x-1">
                                                            {analysisStatus[contact.call_uuid].hasAnalysis ? (
                                                                ...
                                                                <div className="flex flex-col">
                                                                    <div
                                                                        className="flex items-center text-xs text-yellow-400 mb-1">
                                                                        <div
                                                                            className="w-2 h-2 rounded-full bg-yellow-400 mr-1.5 animate-pulse"></div>
                                                                        Processing
                                                                    </div>
                                                                    {/* Show what components are available */}
                                                                    {analysisStatus[contact.call_uuid].details && (
                                                                        <div className="flex space-x-2 text-xs">
                                        <span className={analysisStatus[contact.call_uuid].details.transcript ?
                                            "text-green-400" : "text-gray-500"}>
                                          <FileText size={10} className="inline mr-1"/>
                                          Transcript
                                        </span>
                                                                            <span
                                                                                className={analysisStatus[contact.call_uuid].details.recording ?
                                                                                    "text-green-400" : "text-gray-500"}>
                                          <FileAudio size={10} className="inline mr-1"/>
                                          Audio
                                        </span>
                                                                            <span
                                                                                className={analysisStatus[contact.call_uuid].details.summary ?
                                                                                    "text-green-400" : "text-gray-500"}>
