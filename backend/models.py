...
class CallLog(Base):
    ...
    analytics = relationship("CallAnalytics", uselist=False, back_populates="call", cascade="all, delete-orphan") # Added cascade
...
class CallAnalytics(Base):
    ...
    call_id = Column(Integer, ForeignKey('call_logs.id'), nullable=False, unique=True) # Added unique=True assuming one analytics per call #
...
class CallMapping(Base):
    ...
    ultravox_call_id = Column(String(255), unique=True, nullable=True, index=True) # Allow nullable for calls before mapping #
