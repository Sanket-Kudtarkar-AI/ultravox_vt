...
class Campaign(Base):
    ...
    progress = Column(Integer, default=0)  # Added: Overall progress percentage (0-100)
    analysis_progress = Column(Integer, default=0)  # Added: Analysis progress percentage (0-100)
    ...
    def to_dict(self):
        ...
        return {
            ...
            "config": config_data,
            "progress": self.progress,  # Added: Include progress in dict
            "analysis_progress": self.analysis_progress  # Added: Include analysis_progress in dict
        }
...
class CallAnalysisStatus(Base):
    """
    Model to store call analysis status for each component (transcript, recording, summary)
    """
    __tablename__ = 'call_analysis_status'

    id = Column(Integer, primary_key=True)
    call_uuid = Column(String(255), unique=True, nullable=False, index=True)
    ultravox_id = Column(String(255), index=True)
    ...
    # Overall status
    is_complete = Column(Boolean, default=False)  # True when all components are available
    ...
    def __repr__(self):
        return f"<CallAnalysisStatus call_uuid={self.call_uuid} complete={self.is_complete}>"
